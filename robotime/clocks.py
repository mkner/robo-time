...
class Clock(Object):
    ...
    def __init__(self):
        ...
        #clock no longer references unix epoch, other clocks do
        ...
        #new 9/25 - could call function, but just keep it clean in init
        self._saved_timestamp_date_sep = self._timestamp_date_sep
        self._saved_timestamp_dt_sep = self._timestamp_dt_sep
        self._saved_timestamp_time_sep = self._timestamp_time_sep
        self._saved_timestamp_res_sep = self._timestamp_res_sep 
        ...
        # are here as framework for descendant classes
        self._cal_year = 0
        self._cal_month = 0
        self._cal_day = 0
    ...
    def getMonotime(self): #getMonotime(self):
        ...
        # from some time since the system board started running
    ...
    #mk
    # timing functions
    # these are duplicated from time.py
    ...
    # uptime functions 
    # important!! relative to the init time of 
    # the object instantation of this object!!!!
    # not the OS/HW startup like the freestanding versionsn in time.py
    ...
    def getUptime(self):
        ...
        #0.1.10a
        #mk handled in _getUptimeTuple() where it should be
        #self._updateUptime()
    ...
    def epoch(self):
        # mk it easy, it doesnt use UNIX epoch
        print("0000-00-00 00:00:00")
    ...
    def setTimestampRes(self,res):
        
       # print("setting timestamp res to: ",x)
        
        if ((res == 'micro') or (res == 'milli') or (res == 'none')):
            self._timestamp_res = res
        #otherwise do nothing
        return
    
    def setTimestampDateSep(self,dsep):
        # separator for  date YYYY-MM-DD part
         if (len(dsep) <= 3): #limit for now
             self._timestamp_date_sep = dsep
             
           
    def setTimestampSep(self,dtsep):
        # what is used to between output 
        # of date YYYY-MM-DD and the hh:mm:ss time
        
        if (len(dtsep) <= 3): #limit for now
         self._timestamp_dt_sep = dtsep
    
    #mk
    #NEW 
    def setTimestampTimeSep(self,tsep):
        # what is used for ':"
        # in time string hh:mm:ss 
        
        if (len(tsep) <= 3): #limit for now
         self._timestamp_time_sep = tsep
    
    
    def setTimestampResSep(self,rsep):
        
        if (len(rsep) <= 3): #limit for now
         self._timestamp_res_sep = rsep
         
    #mk
    #NEW    
    def _saveTimestampDelimiters(self):
        # used by set compressed et al
        self._saved_timestamp_date_sep = self._timestamp_date_sep
        self._saved_timestamp_dt_sep = self._timestamp_dt_sep
        self._saved_timestamp_time_sep = self._timestamp_time_sep
        self._saved_timestamp_res_sep = self._timestamp_res_sep 
    ...
    def _restoreTimestampDelimiters(self):
        # used by set compressed et al
        self._timestamp_date_sep = self._saved_timestamp_date_sep
        self._timestamp_dt_sep = self._saved_timestamp_dt_sep
        self._timestamp_time_sep = self._saved_timestamp_time_sep
        self._timestamp_res_sep = self._saved_timestamp_res_sep 
 
    
    def setTimestampCompressedOn(self):
        # make all delimiter/separators ''
        # to compress into compact ascii string
        if self._timestamp_compressed == False:
             #NEW 9/25
             # SAVE CURRENT delimiter settings 
             self._saveTimestampDelimiters()
    ...
    def setTimestampCompressedOff(self):
         #FUTURE is NOW 9/25 preserve prev settings
         if self._timestamp_compressed == True:
             #NEW 9/25
             #RESTORE previous delimiter settings 
             self._restoreTimestampDelimiters()
             self._timestamp_compressed = False
    #mk    
    #use this new semantic- no ed
    def setTimestampCompressOn(self):
        self.setTimestampCompressedOn()
        
    #use this new semantic- no ed
    def setTimestampCompressOff(self):
        self.setTimestampCompressedOff()
    ...
    def getTimestampInt(self):
    # explicit INT name now
    ...
    def getTimestampStr(self):
        ...
        y,mo,dy,h,m,s,res= self.getTimestampInt()
    ...
    #mk
    #CHANGED, 9/26 0.1.08c
    #default now fetch STRing version
    
    # this Clock class here
    #FUTURE may let to set type in use at moment
    #MAKE AN ERROR TO SEE IF DERIVED CLASSES CALL BACK TO?
    #def getTimestamp(self):
    ...
    def now(self):
        ...
        y,mo,dy,h,m,s,res = self.getTimestampInt() #use Int vers
        #y,mo,dy,h,m,s,res = self.getTimestamp()
...
class SystemClock(Clock):
    ...
    #def getTimestamp(self):
      ...
      # now y,mo,dy,h,m,s,res= self.getTimestampInt()
      # was y,mo,dy,h,m,s,res= self.getTimestamp()
...
class WorldClock(Clock):
    ...
    def getTimestampInt(self):
      ...
      # the other timestamp functions call this one
      # now y,mo,dy,h,m,s,res= self.getTimestampInt()
      # was ,mo,dy,h,m,s,res= self.getTimestamp()
      # based on system settings
    ...
    def time(self):
        ...
        # HH:MM:SS
        (yr,mo,dy,h,m,s,res)=self.getTimestampInt() #use int vers
    ...
    def date(self):
        # current year up
        # YYYY-DD-MM
        ts = self.getTimestampInt() # use ...Int()
        #ts = self.getTimestamp()
        yyyy=str(ts[0]).zfill(4)
        mm = str(ts[1]).zfill(2)
        dd = str(ts[2]).zfill(2)
